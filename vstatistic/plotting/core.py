...
def plot_returns_bars(
    ...
    if resample is not None:
        df = df.resample(resample).apply(core.comp).resample(resample).last()
    # ---------------
...
def plot_rolling_beta(
    ...
    i = 1
    if isinstance(returns, _pd.Series):
        beta = core.rolling_greeks(returns, benchmark, window1)["beta"].fillna(0)
        ax.plot(beta, lw=lw, label=window1_label, color=colors[1])
    elif isinstance(returns, _pd.DataFrame):
        beta = {
            col: core.rolling_greeks(returns[col], benchmark, window1)["beta"].fillna(
                0
            )
    ...
    if window2:
        ...
        if isinstance(returns, _pd.Series):
            ax.plot(
                core.rolling_greeks(returns, benchmark, window2)["beta"],
                lw=lw,
                label=window2_label,
                color="gray",
                alpha=0.8,
            ...
            betas_w2 = {
                col: core.rolling_greeks(returns[col], benchmark, window2)["beta"]
                for col in returns.columns
            }
...
def plot_distribution(
    ...
    port["Monthly"] = port["Daily"].resample(MONTH_END).apply(apply_fnc)
    port["Monthly"] = port["Monthly"].ffill()

    port["Quarterly"] = port["Daily"].resample(QUARTER_END).apply(apply_fnc)
    port["Quarterly"] = port["Quarterly"].ffill()

    port["Yearly"] = port["Daily"].resample(YEAR_END).apply(apply_fnc)
